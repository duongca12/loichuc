...
def read_proxy_file(filename):
    try:
        ...
        return []
def ensure_file(path: str, default_text: str = ""):
    """Tạo file nếu chưa có (ghi nội dung mặc định nếu muốn)."""
    if not os.path.exists(path):
        with open(path, "w", encoding="utf8") as f:
            f.write(default_text)

def parse_proxy_line(s: str):
    """
    Hỗ trợ 2 dạng:
      - host:port:user:pass
      - user:pass@host:port
    Trả về dict proxies dùng cho requests hoặc None nếu không hợp lệ.
    """
    s = s.strip()
    if not s:
        return None
...
def Delay(value):
    ...
    for i in range(value, 0, -1):
        sys.stdout.write(f"\rChờ {i}  ")
        sys.stdout.flush()
        time.sleep(1)
    sys.stdout.write("\r" + " " * 70 + "\r")  # clear dòng
...
class Facebook_Api(object):
        ...
        try:
            ...
            if proxy:
                try:
                    ...
                    self.proxies = proxy
            url = requests.get(f'https://www.facebook.com/{self.actor_id}', headers=self.headers,
    ...
    def info(self):
        ...
        if str(self.actor_id) in get:
            try:
                name = re.search(r"<title>(.*?)</title>", get)
                if name:
                    return {'success': 200, 'id': self.actor_id, 'name': name.group(1)}
                return {'success': 200, 'id': self.actor_id, 'name': "hehe"}
            except Exception as e:
                traceback.print_exc()
                return {'error': 200, "msg": e}
        else:
            return {'error': 200, "msg": "Cookie Die"}
    ...
    def Checkspam(self):
        ...
        try:
            response = requests.post('https://www.facebook.com/api/graphql/', headers=self.headers, data=data,
    ...
    def reactioncmt(self, id, type):
        ...
        data = {
            ...
            'fb_api_caller_class': 'RelayModern',
            'fb_api_req_friendly_name': 'CometUFIFeedbackReactMutation',
            'variables': '{"input":{"attribution_id_v2":"CometVideoHomeNewPermalinkRoot.react,comet.watch.injection,via_cold_start,1719930662698,975645,2392950137,,","feedback_id":"' + encode_to_base64(
                "feedback:" + str(
                    id)) + '","feedback_reaction_id":"' + id_reac + '","feedback_source":"TAHOE","is_tracking_encrypted":true,"tracking":[],"session_id":"' + str(
                uuid.uuid4()) + '","downstream_share_session_id":"' + str(
                uuid.uuid4()) + '","downstream_share_session_origin_uri":"https://fb.watch/t3OatrTuqv/?mibextid=Nif5oz","downstream_share_session_start_time":"' + starttime + '","actor_id":"' + self.actor_id + '","client_mutation_id":"1"},"useDefaultActor":false,"__relay_internal__pv__CometUFIReactionsEnableShortNamerelayprovider":false}',
            'server_timestamps': 'true',
    ...
    def share(self, id):
        data = {
            ...
            'variables': '{"input":{"composer_entry_point":"share_modal","composer_source_surface":"feed_story","composer_type":"share","idempotence_token":"' + str(
    ...
    def like_page(self, id):
        data = {
            ...
            'variables': '{"input":{"is_tracking_encrypted":false,"page_id":"' + str(
                id) + '","source":null,"tracking":null,"actor_id":"' + str(
                self.actor_id) + '","client_mutation_id":"1"},"scale":1}',
            'server_timestamps': 'true',
    ...
    def follow(self, id):
        data = {
            ...
            'variables': '{"input":{"attribution_id_v2":"ProfileCometTimelineListViewRoot.react,comet.profile.timeline.list,unexpected,1719765181042,489343,250100865708545,,;SearchCometGlobalSearchDefaultTabRoot.react,comet.search_results.default_tab,unexpected,1719765155735,648442,391724414624676,,;SearchCometGlobalSearchDefaultTabRoot.react,comet.search_results.default_tab,tap_search_bar,1719765153341,865155,391724414624676,,","is_tracking_encrypted":false,"subscribe_location":"PROFILE","subscribee_id":"' + str(
                id) + '","tracking":null,"actor_id":"' + str(self.actor_id) + '","client_mutation_id":"5"},"scale":1}',
            'server_timestamps': 'true',
        ...
        try:
            ...
            return False  # http://14.225.59.13:12345:VN27377:kStL9j8s
...
class TraoDoiSub_Api(object):
    ...
    def add_uid(self, id, g_recaptcha_response):
        ...
        response = self.session.post('https://traodoisub.com/scr/add_uid.php', headers=headers,
                                     data={'idfb': id, 'g-recaptcha-response': g_recaptcha_response},
    ...
    def get_nv_thuong(self, fields):
        try:
            list_nv = self.session.get(f'https://traodoisub.com/api/?fields={fields}&access_token={self.token}',
                                       proxies=self.proxies).json()
            return list_nv
        except:
            return False
    ...
    def get_xu_vip(self, type, id):
        try:
            get_xu = self.session.get(f'https://traodoisub.com/api/coin/?type={type}&id={id}&access_token={self.token}',
                                      proxies=self.proxies).json()
            return get_xu
        except:
            return False
    ...
    def cache(self, type, id):
        try:
            cache = self.session.get(f'https://traodoisub.com/api/coin/?type={type}&id={id}&access_token={self.token}',
                                     proxies=self.proxies).json()
            return cache
        except:
            return False
...
def Nhap_Cookie(proxy_list=None):
    """
    Đọc/nhập danh sách tài khoản FB từ Cookie_FB.txt.
    Format mỗi dòng (đúng theo code đang dùng GETTOKEN):
        user|pass|2FA
    - Tự tạo file nếu chưa có.
    - Nếu file rỗng, cho phép nhập nhanh trong CLI rồi lưu lại.
    """
    ensure_file("Cookie_FB.txt", "")
    # Đọc file
    with open("Cookie_FB.txt", "r", encoding="utf8") as f:
        lines = [line.strip() for line in f if line.strip()]

    # Nếu file rỗng → hỏi nhập nhanh để tiện dùng luôn
    if not lines:
        print(f"{thanh} {luc}Chưa có tài khoản trong {vang}Cookie_FB.txt{trang}.")
        print(f"{thanh} {luc}Nhập dạng {vang}user|pass|2FA{trang}. {lam}(Enter trống để kết thúc){trang}")
        collected = []
        while True:
            s = input("> ")
            if not s.strip():
        ...
        with open("Cookie_FB.txt", "w", encoding="utf8") as f:
            f.write("\n".join(collected))
        lines = collected

    # Nếu vẫn rỗng → trả về list rỗng để caller xử lý
    if not lines:
        print(f"{thanh} {red}Không có tài khoản nào, vui lòng thêm vào Cookie_FB.txt.")
        return []

    try:
        sl = int(input(f"Tìm Thấy {len(lines)} acc, Nhập Số Lượng muốn chạy: "))
    except:
        sl = len(lines)

    # Dùng proxy nếu có; còn không thì để None
    selected = lines[:max(0, min(sl, len(lines)))]
    for idx, data in enumerate(selected, 1):
        # Parse tài khoản
        parts = data.split("|")
        user = (parts[0] if len(parts) > 0 else "").strip()
        pas  = (parts[1] if len(parts) > 1 else "").strip()
        f2fa = (parts[2] if len(parts) > 2 else "").strip()

        # Lấy proxy (nếu có danh sách)
        proxies = None
        if proxy_list:
            try:
                proxy_str = random.choice(proxy_list).strip()
                proxies = parse_proxy_line(proxy_str)
            except Exception:
                proxies = None

        print(f"Check Acc {idx}/{len(selected)}: {user} Bắt Đầu Login!")
        tam = GETTOKEN(user, pas, fa=f2fa, proxy=proxies)
        infoFB = tam if tam else None

        if infoFB:
            fb = Facebook_Api(infoFB["cookie"], proxy=proxies)
            info = fb.info()
            ...
            print(f"Acc[{user}] {idx}/{len(selected)}: Không Login Được")
...
def Main():
    ...
    if proxy_list:
        ...
        host, port, username, password = current_proxy.split(":")
        data = {
            'listProxy': [
                {
                    'username': username,
    ...
    while True:
        ...
        if not os.path.exists('acc_tds_log.txt'):
            username, password = input(f'{thanh} {luc}Nhập Tài Khoản TDS{trang}: '), input(
    ...
    thanhngang(70)
    while True:
        if os.path.exists('Cookie_FB2.txt'):
            ...
            if chon == '1':
                ...
                with open('Cookie_FB2.txt', 'r') as f:
                    listck = json.loads(f.read())
                    break
            elif chon == '2':
                os.remove('Cookie_FB2.txt')
            else:
                print(f'{thanh} {red}Vui Lòng Chọn Đúng')
                thanhngang(70)
                continue
        if not os.path.exists('Cookie_FB2.txt'):
            listck = Nhap_Cookie(proxy_list)
            with open('Cookie_FB2.txt', 'w') as f:
                json.dump(listck, f)
            break
    ...
    while True:
        ...
        for ck in listck:
            ...
            if 'success' in info:
                ...
                print(f'{red}Cookie Tài Khoản {uid} Die');
                sleep(0);
                print('                                     ')
                listck.remove(ck)
                continue
            ...
            while True:
                ...
                if '1' in nhiemvu:
                    ...
                    if listcx == False:
                        print(f'{luc}Không Get Được Nhiệm Vụ Cảm Xúc                           ');
                        sleep(2);
                        print('                                                        ')
                        ...
                        list_nv = listcx['data']
                        if len(list_nv) == 0:
                            print(f'{luc}Hết Nhiệm Vụ Cảm Xúc                           ');
                        else:
                            print(f'{luc}Tìm Thấy {vang}{len(list_nv)} {luc}Nhiệm Vụ Cảm Xúc                      ')
                            for x in list_nv:
                                idpost = x['id']
                                ...
                                if like == False:
                                    print(f"{luc}FAIL {lam}{type}{trang}: {vang}{id}            ", end='\r');
                                    loireaction += 1
                                else:
                ...
                if '2' in nhiemvu:
                    ...
                    if listcxcmt == False:
                        print(f'{luc}Không Get Được Nhiệm Vụ Cảm Xúc Cmt                           ');
                        sleep(0);
                        print('                                                        ')
                        ...
                        if listcxcmt['error'] == 'Thao tác quá nhanh vui lòng chậm lại':
                            ...
                            print(listcxcmt['error']);
                            sleep(0);
                            print('                                                       ')
                    else:
                        list_nv = listcxcmt['data']
                        if len(list_nv) == 0:
                            print(f'{luc}Hết Nhiệm Vụ Cảm Xúc Cmt                           ');
                        else:
                            print(f'{luc}Tìm Thấy {vang}{len(list_nv)} {luc}Nhiệm Vụ Cảm Xúc Cmt                     ')
                            for x in list_nv:
                                idpost = x['id']
                                id = idpost.split('_')[1] if '_' in idpost else idpost
                                id2 = id[:3] + '#' * (len(id) - 6) + id[-3:]
                                ...
                                if like == False:
                                    print(f"{luc}FAIL {lam}{type}CMT{trang}: {vang}{id}            ", end='\r');
                                    loicxcmt += 1
                                else:
                ...
                if '3' in nhiemvu:
                    ...
                    if listshare == False:
                        print(f'{luc}Không Get Được Nhiệm Vụ Share                           ');
                        sleep(0);
                        print('                                                        ')
                        ...
                        if listshare['error'] == 'Thao tác quá nhanh vui lòng chậm lại':
                            ...
                            print(listshare['error']);
                            sleep(0);
                            print('                                                       ')
                    else:
                        list_nv = listshare['data']
                        if len(list_nv) == 0:
                            print(f'{luc}Hết Nhiệm Vụ Share                           ');
                        else:
                            print(f'{luc}Tìm Thấy {vang}{len(list_nv)} {luc}Nhiệm Vụ Share                     ')
                        for x in list_nv:
                            idpost = x['id']
                            id = idpost.split('_')[1] if '_' in idpost else idpost
                            id2 = id[:3] + '#' * (len(id) - 6) + id[-3:]
                            ...
                            like = fb.share(id)
                            if like == False:
                                print(f"{luc}FAIL {lam}SHARE{trang}: {vang}{id}            ", end='\r');
                                loishare += 1
                            else:
                ...
                if '4' in nhiemvu:
                    ...
                    if listfl == False:
                        print(f'{luc}Không Get Được Nhiệm Vụ Follow                           ');
                        sleep(0);
                        print('                                                        ')
                        ...
                        list_nv = listfl['data']
                        if len(list_nv) == 0:
                            print(f'{luc}Hết Nhiệm Vụ Follow                           ');
                        else:
                            print(f'{luc}Tìm Thấy {vang}{len(list_nv)} {luc}Nhiệm Vụ Follow                     ')
                            for x in list_nv:
                                idpost = x['id']
                                id = idpost.split('_')[1] if '_' in idpost else idpost
                                id2 = id[:3] + '#' * (len(id) - 6) + id[-3:]
                                ...
                                like = fb.follow(id)
                                if like == False:
                                    print(f"{luc}FAIL {lam}FOLLOW{trang}: {vang}{id}            ", end='\r');
                                    loifollow += 1
                                else:
                ...
                if '5' in nhiemvu:
                    ...
                    if listpage == False:
                        print(f'{luc}Không Get Được Nhiệm Vụ Like Page                           ');
                        sleep(0);
                        print('                                                        ')
                        ...
                        if listpage['error'] == 'Thao tác quá nhanh vui lòng chậm lại':
                            ...
                            print(listpage['error']);
                            sleep(0);
                            print('                                                       ')
                    else:
                        list_nv = listpage['data']
                        if len(list_nv) == 0:
                            print(f'{luc}Hết Nhiệm Vụ Like Page                           ');
                        else:
                            print(f'{luc}Tìm Thấy {vang}{len(list_nv)} {luc}Nhiệm Vụ Like Page                     ')
                            for x in list_nv:
                                idpost = x['id']
                                id = idpost.split('_')[1] if '_' in idpost else idpost
                                id2 = id[:3] + '#' * (len(id) - 6) + id[-3:]
                                ...
                                if like == False:
                                    print(f"{luc}FAIL {lam}LIKEPAGE{trang}: {vang}{id}            ", end='\r');
                                    loipage += 1
                                else:
                ...
                if '6' in nhiemvu:
                    ...
                    if listgr == False:
                        print(f'{luc}Không Get Được Nhiệm Vụ Group                           ');
                        sleep(0);
                        print('                                                        ')
                        ...
                        list_nv = listgr
                        if len(list_nv) == 0:
                            print(f'{luc}Hết Nhiệm Vụ Group                           ');
                        else:
                            print(f'{luc}Tìm Thấy {vang}{len(list_nv)} {luc}Nhiệm Vụ Group                     ')
                        for x in list_nv:
                            idpost = x['id']
                            id2 = idpost[:3] + '#' * (len(idpost) - 6) + idpost[-3:]
                            like = fb.group(idpost)
                            if like == False:
                                print(f"{luc}FAIL {lam}GROUP{trang}: {vang}{idpost}            ", end='\r');
                                loigr += 1
                            else:
                ...
                if '7' in nhiemvu:
                    ...
                    if listshare == False:
                        print(f'{luc}Không Get Được Nhiệm Vụ Share Thường                           ');
                        sleep(0);
                        print('                                                        ')
                    elif 'error' in listshare:
                        if listshare['error'] == 'Thao tác quá nhanh vui lòng chậm lại':
                        ...
                        list_nv = listshare
                        if len(list_nv) == 0:
                            print(f'{luc}Hết Nhiệm Vụ Share Thường                           ');
                        else:
                            print(f'{luc}Tìm Thấy {vang}{len(list_nv)} {luc}Nhiệm Vụ Share Thường                     ')
                        for x in list_nv:
                            idpost = x['id']
                            id2 = idpost[:3] + '#' * (len(idpost) - 6) + idpost[-3:]
                            share = fb.share(idpost)
                            if share == False:
                                print(f"{luc}FAIL {lam}SHARE{trang}: {vang}{idpost}            ", end='\r');
                                loisharet += 1
                            else:
                ...
                if '8' in nhiemvu:
                    ...
                    if listpage == False:
                        print(f'{luc}Không Get Được Nhiệm Vụ Like Page Thường                          ');
                        sleep(0);
                        print('                                                        ')
                    elif 'error' in listpage:
                        if listpage['error'] == 'Thao tác quá nhanh vui lòng chậm lại':
                        ...
                        list_nv = listpage
                        if len(list_nv) == 0:
                            print(f'{luc}Hết Nhiệm Vụ Like Page Thường                          ');
                        else:
                            print(
                            ...
                            for x in list_nv:
                                ...
                                if like == False:
                                    print(f"{luc}FAIL {lam}LIKEPAGE{trang}: {vang}{idpost}            ", end='\r');
                                    loipage += 1
                                else:
                ...
                if '9' in nhiemvu:
                    ...
                    if listlike == False:
                        print(f'{luc}Không Get Được Nhiệm Vụ Like Thường                          ');
                        sleep(0);
                        print('                                                        ')
                        ...
                        if listlike['error'] == 'Thao tác quá nhanh vui lòng chậm lại':
                            ...
                            print(listlike['error']);
                            sleep(0);
                            print('                                                       ')
                    else:
                        list_nv = listlike
                        if len(list_nv) == 0:
                            print(f'{luc}Hết Nhiệm Vụ Like Thường                          ');
                        else:
                            print(f'{luc}Tìm Thấy {vang}{len(list_nv)} {luc}Nhiệm Vụ Like Thường                    ')
                            for x in list_nv:
                                idpost = x['id']
                                id = idpost.split('_')[1] if '_' in idpost else idpost
                                id2 = id[:3] + '*' * (len(id) - 6) + id[-3:]
                                ...
                                if like == False:
                                    print(f"{luc}FAIL {lam}LIKETHUONG{trang}: {vang}{id}            ", end='\r');
                                    loiliket += 1
                                else:
                ...
                if '0' in nhiemvu:
                    ...
                    if listlike == False:
                        print(f'{luc}Không Get Được Nhiệm Vụ Cảm Xúc Thường                          ');
                        sleep(0);
                        print('                                                        ')
                    elif 'error' in listlike:
                        if listlike['error'] == 'Thao tác quá nhanh vui lòng chậm lại':
                        ...
                        list_nv = listlike
                        if len(list_nv) == 0:
                            print(f'{luc}Hết Nhiệm Vụ Cảm Xúc Thường                          ');
                        else:
                            print(
...
def GETTOKEN(tk, mk, fa=None, token="275254692598279|585aec5b4c27376758abb7ffcb9db2af", proxy=None):
    xnhac = "\033[1;36m"
    do = "\033[1;31m"
    luc = "\033[1;32m"
    vang = "\033[1;33m"
    ...
    "origin":"https://b-graph.facebook.com",
    ...
    try:
        LOGIN = requests.post("https://b-graph.facebook.com/auth/login", headers=head, data=data, proxies=proxy if proxy else None).json()
        print(xduong+"Đang Login...")
    except:
        print(do+"Không Login Được")
        return None
    try:
        Token=LOGIN['access_token']
        c_user=LOGIN['session_cookies'][0]['value']
        xs=LOGIN['session_cookies'][1]['value']
        fr=LOGIN['session_cookies'][2]['value']
        datr=LOGIN['session_cookies'][3]['value']
        cookie = "datr="+datr+"; fr="+fr+"; xs="+xs+"; c_user="+c_user+"; locale=vi_VN; m_page_voice="+c_user+"; _uafec=Mozilla%2F5.0%20%28Linux%3B%20Android%2012%3B%20SM-A217F%29%20AppleWebKit%2F537.36%20%28KHTML%2C%20like%20Gecko%29%20Chrome%2F109.0.0.0%20Mobile%20Safari%2F537.36; useragent=TW96aWxsYS81LjAgKExpbnV4OyBBbmRyb2lkIDEyOyBTTS1BMjE3RikgQXBwbGVXZWJLaXQvNTM3LjM2IChLSFRNTCwgbGlrZSBHZWNrbykgQ2hyb21lLzEwOS4wLjAuMCBNb2JpbGUgU2FmYXJpLzUzNy4zNg=="
        print(vang+"Login Success!")
        return {"cookie":cookie,"token":Token}
    except:
        try:
            if LOGIN["error"]["error_user_title"] == "Bạn hiện không thể đăng nhập":
                print(do+"CHÚNG TÔI ĐÃ TẠM THỜI KHÓA TÀI KHOẢN CỦA BẠN ĐỂ ĐẢM BẢO AN TOÀN. TRƯỚC KHI BẠN THỬ ĐĂNG NHẬP LẠI, HÃY KIỂM TRA THÔNG TIN ĐĂNG NHẬP VÀ CHẮC CHẮN LÀ BẠN ĐANG SỬ DỤNG THIẾT BỊ THƯỜNG DÙNG TRÊN MẠNG AN TOÀN.")
            elif LOGIN["error"]["code"] == "368":
                print(do+"BẠN ĐÃ GET QUÁ NHIỀU LẦN, VUI LÒNG GET LẠI SAU VÀI GIỜ")
            elif LOGIN['error']['error_subcode'] == "1348077" or LOGIN["error"]["error_user_title"] == "Email không chính xác" or LOGIN["error"]["message"] == "Invalid username or email address" or LOGIN["error"]["error_user_title"] == "Sai mật khẩu" or LOGIN["error"]["message"] == "Invalid username or password":
                print(do+"TÀI KHOẢN FACEBOOK KHÔNG TỒN TẠI")
            elif LOGIN['error']['error_user_title'] == "Cần có mã đăng nhập" or LOGIN["error"]["error_user_title"] == "Login Code Required" or LOGIN["error"]["error_subcode"] == "1348162":
                if fa == "":
                    print(do+"FACEBOOK CẦN CÓ 2FA")
                codefa = fa.replace(' ','')
                print(vang+"Đang Lấy 2fa...")
                get = requests.get("https://2fa.live/tok/"+codefa).json()
                print(xduong+"Lấy 2fa thành công!")
                factor = LOGIN['error']['error_data']['login_first_factor']
                uid2Fa = LOGIN['error']['error_data']['uid']
                data = {
                "email":tk,
                "password":get["token"],
                "sim_serials":"[]",
                ...
                "locale":"vi_VN",
                "client_country_code":"VN",
                "twofactor_code":get["token"],
                "userid":uid2Fa,
                "first_factor":factor,
                "access_token":token
                }
                print(xduong + "Đang Login...")
                LOGINFA = requests.post("https://b-graph.facebook.com/auth/login", headers=head, data=data, proxies=proxy if proxy else None).json()
                print(vang + "Login Success!")
                Token=LOGINFA['access_token']
                c_user=LOGINFA['session_cookies'][0]['value']
                xs=LOGINFA['session_cookies'][1]['value']
                fr=LOGINFA['session_cookies'][2]['value']
                datr=LOGINFA['session_cookies'][3]['value']
                cookie = "datr="+datr+"; fr="+fr+"; xs="+xs+"; c_user="+c_user+"; locale=vi_VN; m_page_voice="+c_user+"; _uafec=Mozilla%2F5.0%20%28Linux%3B%20Android%2012%3B%20SM-A217F%29%20AppleWebKit%2F537.36%20%28KHTML%2C%20like%20Gecko%29%20Chrome%2F109.0.0.0%20Mobile%20Safari%2F537.36; useragent=TW96aWxsYS81LjAgKExpbnV4OyBBbmRyb2lkIDEyOyBTTS1BMjE3RikgQXBwbGVXZWJLaXQvNTM3LjM2IChLSFRNTCwgbGlrZSBHZWNrbykgQ2hyb21lLzEwOS4wLjAuMCBNb2JpbGUgU2FmYXJpLzUzNy4zNg=="
                return {"cookie":cookie,"token":Token}
            else:
                return None
...
Main()
# data = "61579323557091|Vinhraufaptv1992|D2UM QKV6 HEBS Z7FP HMCJ 74S6 HAZE EEGO|LoreleiBarratt466@hotmail.com|KfIcnhVy6A5"
# user = data.split("|")[0].strip()
# pas = data.split("|")[1].strip()
# f = data.split("|")[2].strip()
# tam = GETTOKEN(user, pas, fa=f)
# infoFB = tam if tam else None
# if infoFB:
#     fb = Facebook_Api(infoFB["cookie"])
#     print(fb.info())